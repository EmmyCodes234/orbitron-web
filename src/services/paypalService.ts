...
/**
 * Process a completed custom PayPal payment
 * @param paymentId - The PayPal payment ID
 * @param payerId - The PayPal payer ID
 ...
 * @returns Boolean indicating success or failure
 */
export const processCompletedCustomPayment = async (
  paymentId: string,
  payerId: string,
  ...
  try {
    ...
    if (verification.amount !== expectedAmount) {
      console.error('Payment amount mismatch. Expected $' + expectedAmount + ', got $' + verification.amount);
      return false;
    }

    // Verify the currency is correct
    if (verification.currency !== expectedCurrency) {
      console.error('Payment currency mismatch. Expected ' + expectedCurrency + ', got ' + verification.currency);
      return false;
    }

    console.log(`Custom payment of ${expectedAmount} ${expectedCurrency} processed successfully for payment ${paymentId}`);

    return true;
  } catch (error) {
    console.error('Error processing completed custom payment:', error);
    return false;
  }
};
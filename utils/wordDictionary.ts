...
export function solveWordBuilder(letters: string): Record<number, string[]> {
  ...
  // Check each word length from 2 up to input length
  // We start from 2 because 1-letter words are usually not valid in Scrabble-like games
  // but if needed we could start from 1.
  const minLength = 2; // Was upperLetters.length - blankCount which restricted to near-anagrams

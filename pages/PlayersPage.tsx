...
const PlayersPage: React.FC = () => {
  ...
  // Filter players based on search term and selected country
          ...
          <div className="overflow-x-auto">
            <table className="w-full font-jetbrains-mono text-xs sm:text-sm text-left min-w-full">
              <thead className="bg-slate-800/50 text-xs text-green-400 uppercase tracking-wider">
                <tr>
                  <th scope="col" className="px-4 sm:px-6 py-3 text-center">Rank</th>
                  <th scope="col" className="px-4 sm:px-6 py-3">Player</th>
                  <th scope="col" className="px-4 sm:px-6 py-3">Country</th>
                  <th scope="col" className="px-4 sm:px-6 py-3 text-center">Rating</th>
                  <th scope="col" className="px-4 sm:px-6 py-3 text-center hidden md:table-cell">Events</th>
                </tr>
              </thead>
              <tbody>
                {filteredPlayers.length > 0 ? (
                  filteredPlayers.map((player, index) => (
                    <tr key={`${player.nick}-${index}`} className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 sm:px-6 py-3 font-bold text-white whitespace-nowrap">{player.name}</td>
                      <td className="px-4 sm:px-6 py-3 text-gray-400">{player.country}</td>
                      <td className="px-4 sm:px-6 py-3 text-center text-cyan-400">{player.rating}</td>
